"""
Command-Line Interface for EpisodicVector-Store: Hierarchical HNSW & Cross-Session Memory Agent.
"""
import argparse
import csv
import json
...
from .agents import EpisodicMemoryCoordinator

coordinator = EpisodicMemoryCoordinator()


def main(argv=None):
    parser = argparse.ArgumentParser(prog="long-term-episodic-vector-db", description="EpisodicVector-Store: Hierarchical HNSW & Cross-Session Memory Agent")
    ...
    if args.command == "audit":
        ...
        print("=" * 80)
        print(f"  EPISODICVECTOR-STORE: HIERARCHICAL HNSW & CROSS-SESSION MEMORY AGENT")
        print(f"  Domain: Autonomous Context Management & State Engines | Standard: HNSW Vector Indexing Specification")
    ...
    if args.command == "serve":
        try:
            ...
            app = create_app()
            if app:
                print(f"Starting EpisodicVector-Store: Hierarchical HNSW & Cross-Session Memory Agent on http://{args.host}:{args.port}")
